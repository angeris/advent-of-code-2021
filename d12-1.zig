...
pub fn main() !void {
    ...
    const total = countPaths(nmap, isupper.items, startidx, visited, endidx);

    print("total {}\n", .{total});
}

fn countPaths(nmap: ArrayList(ArrayList(usize)), isupper: []bool, currpos: usize, visited: []bool, end: usize) usize {
    if (!isupper[currpos] and visited[currpos]) return 0;
    ...
    for (nmap.items[currpos].items) |neighbor| {
        total += countPaths(nmap, isupper, neighbor, visited, end);
    }
